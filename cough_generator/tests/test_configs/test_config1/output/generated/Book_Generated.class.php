...
abstract class Book_Generated extends CoughObject {
	...
	public function loadAuthor_Object() {
		$author = Author::constructByKey(array(
			'`author`.`author_id`' => $this->getAuthorId(),
		));
		$this->setAuthor_Object($author);
	}
	...
	public function loadBook2library_Collection() {
		...
		if ($this->hasKeyId()) {
			...
			$sql = '
				...
				WHERE
					`book2library`.`book_id` = ' . $db->quote($this->getBookId()) . '
			';
