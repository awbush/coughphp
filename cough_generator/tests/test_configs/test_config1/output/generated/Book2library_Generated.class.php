...
abstract class Book2library_Generated extends CoughObject {
	...
	public static function getLoadSql() {
		return '
			SELECT
				`book2library`.*
			FROM
				`test_cough_object`.`book2library`
		';
	}
	...
	public function loadBook_Object() {
		$book = Book::constructByKey(array(
			'`book`.`book_id`' => $this->getBookId(),
		));
		$this->setBook_Object($book);
	}
	...
	public function loadLibrary_Object() {
		$library = Library::constructByKey(array(
			'`library`.`library_id`' => $this->getLibraryId(),
		));
		$this->setLibrary_Object($library);
	}
